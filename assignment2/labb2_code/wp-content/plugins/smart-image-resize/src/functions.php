 ...
 * @internal
 */
function _wp_sir_get_size_fit_mode($size_name, $size_options = []) {
    if (empty($size_options)) {
        $size_options = wp_sir_get_settings()['size_options'];
    }

    if (
        !empty($size_options[$size_name]['fit_mode'])
        && in_array($fit_mode = $size_options[$size_name]['fit_mode'], ['contain', 'cover', 'none'])
    ) {
        return $fit_mode;
    }
